...
class FaceRecog:
    ...
    def __init__(self):
        ...
        # self.video = cv2.VideoCapture('./video/video_test2.mp4')
    ...
    def get_frame(self):
        ...
        # 1. realtime 찍어보기 (어디가 딜레이가 심한 부분인지 체크)
        # ret, frame = self.video.read()
        # interval만큼의 frame을 먼저 뽑아오기(배열로, offset이용?)
        # [추후]여러 쓰레드 생성 후 동시에 수행
        if frame is None:
            self.video_end = True
            return None
        ...
        # if self.totalFrame % self.interval == 0:
        ...
        if len(self.face_locations) == 0 and self._working == True:
            # 근무태만이 시작된 시간 저장..
            ...
            for face_name in self.face_names:
                ...
                if face_name == self.name:
                    ...
                    break
            # 프레임 안에 근무자가 있으면 근무중
            if self.workerExist == True and self._working == False:
                ...
                # 근무태만 -> 근무중이 되었을때
                self.alertSlackOff = False
                # 근무태만 시간이 10초가 지난 뒤에 다시 재근무 했을 때
                ...
                # 근무 태만 시간이 10초 되기 전에 재근무 했을 때는 별다른 코드x..
            ...
            elif self.workerExist == False and self._working == True:
                # 근무태만이 시작된 시간 저장..
                self.slackOffStartTime = datetime.now().replace(microsecond=0)
                self.slackOffCount = timeit.default_timer()
